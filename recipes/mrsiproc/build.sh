...
export matlabMCRVersion='2023a' # matlab MCR has to match the version on which the matlab scripts were compiled
export matlabUpdateVersion='7'
export hMRIVersion='0.6.1' # this requires matlab MCR 2023a (Update 1)
...
export mrsi_pipeline_version='86a9de2ac939f3eafc1a1a980b85513896e0abc0'
...
neurodocker generate ${neurodocker_buildMode} \
   ...
   --install language-pack-en libncurses5 gettext xterm x11-apps csh tcsh xorg xorg-dev xserver-xorg-video-intel libwayland-cursor0 \
   ...
   --run="wget https://ssd.mathworks.com/supportfiles/downloads/R${matlabMCRVersion}/Release/${matlabUpdateVersion}/deployment_files/installer/complete/glnxa64/MATLAB_Runtime_R${matlabMCRVersion}_Update_${matlabUpdateVersion}_glnxa64.zip && \
          unzip MATLAB_Runtime_R${matlabMCRVersion}_Update_${matlabUpdateVersion}_glnxa64.zip -d mcrtmp && \
          mcrtmp/install -mode silent -destinationFolder /opt/MATLAB_Runtime_R${matlabMCRVersion} -agreeToLicense yes && \
          rm -rf mcrtmp && \
          rm -f MATLAB_Runtime_R${matlabMCRVersion}_Update_${matlabUpdateVersion}_glnxa64.zip" \
   \
   --env LD_LIBRARY_PATH="\${LD_LIBRARY_PATH}:/opt/MATLAB_Runtime_R${matlabMCRVersion}/R${matlabMCRVersion}/runtime/glnxa64:/opt/MATLAB_Runtime_R${matlabMCRVersion}/R${matlabMCRVersion}/bin/glnxa64:/opt/MATLAB_Runtime_R${matlabMCRVersion}/R${matlabMCRVersion}/sys/os/glnxa64:/opt/MATLAB_Runtime_R${matlabMCRVersion}/R${matlabMCRVersion}/extern/bin/glnxa64" \
   \
   --workdir /opt `# Add hMRI toolbox` \
   --run="wget -q https://github.com/hMRI-group/hMRI-toolbox/releases/download/v${hMRIVersion}/standalone-hMRItoolboxv${hMRIVersion}.zip \
            && unzip standalone-hMRItoolboxv${hMRIVersion}.zip \
            && rm -f standalone-hMRItoolboxv${hMRIVersion}.zip" \
   ...
   --env DEPLOY_BINS="matlab:mex:julia:python:bc:dcm2niix:nii2mnc:dcm2mnc:rawtominc:mincresample:mincmath:minctoraw:mnc2nii:bet:hd-bet:mri_synthseg:lcmodel:parallel:fsl:fslmaths:Part1_ProcessMRSI.sh:Part2_EvaluateMRSI.sh:update_mrsi.sh:fid_1.300000ms.basis:LCModel_Control_Template.m:LCModel_Control_Volunteers_WatRef.m" \
   \
